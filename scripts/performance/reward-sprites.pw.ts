...
for(const [quality,motion,width,height] of [['full','full',1194,834],['lite','full',780,312],['static','full',780,312],['full','reduced',780,312]] as const) test(`WALL04C enlarged rewards preserve real credit and cleanup ${quality}/${motion}`,async({browser})=>{
 ...
 for(const {id,fixture:f} of fixtures){
  ...
  try{
   ...
   await page.addInitScript(({snapshot,quality,motion,keys,progress,preferences})=>{
   ...
   },{snapshot:savedFixture(f,`large-reward-${id}`),quality,motion,keys:{run:ACTIVE_RUN_STORAGE_KEY,progress:PLAYER_PROGRESS_STORAGE_KEY,preferences:PRESENTATION_PREFERENCES_KEY},progress:createDefaultPlayerProgress(16),preferences:DEFAULT_PRESENTATION_PREFERENCES});
   ...
   await page.evaluate(()=>{
    ...
    const sample=()=>{const c=document.querySelector<HTMLCanvasElement>('.vfx-rewards')!;
     samples.push({w:c.width,h:c.height,tokens:Number(c.dataset.tokens)||0,anchors:document.querySelectorAll('[data-reward-anchor="ame"]').length});
