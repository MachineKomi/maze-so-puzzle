...
Current sequence: **01/MOVE-01 engineering history → UI-03 corrected FP-UI1 publication → V22-PERF-01-R1 / v0.22.1 affected-device test → ART-HOTFIX-01 → V22-PLAY-01 → V22-UI-01 / family test → root PT36 preflight → 04 → 02 → remaining UI-02 → 08 → 05 → 07B /
...
| 5C | **ART-HOTFIX-01 — Tessera field-alpha repair** | Starts only after the performance preview's exact bytes have been tested, so the asset change cannot confuse that evidence. It derives a new versioned 256px field rendition from the already approved bounded alpha-recovery master; no character redesign or broad art pass. | Versioned derivative/catalogue/provenance/allocation update; alpha and actual-size proof; cage, follower, Book/victory consumer checks; independent review. |
