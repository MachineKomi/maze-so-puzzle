...
- `src/cameraMotion.ts::worldLayerStyle` maps tile x/y to full-level percentages. PERF-02A keeps `cameraWorldStyle` at layout origin `0%,0%`, still sized by level / camera size. `useSceneTravel` owns the entire sampled world-camera translation as percentages of that full-world box, including first mount, resize and Static mode. This automatically scales with layout changes before ResizeObserver updates cached pixel geometry. Camera-relative actors and anchors retain their compensating pixel delta. Never apply both the historical left/top camera offset and the new absolute translation. This removes per-step layout rebasing, not the full-world rendering cost; affected-iPad acceptance remains open.
