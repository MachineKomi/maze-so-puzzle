...
import { AME_ART, type ArtGeometry } from "./artCatalog";
import { FIELD_GROUND_Y } from "./game/tallWalls";

export type FieldArtRole = "actor" | "item" | "door" | "weapon";
...
export function measureFieldArt(geometry: ArtGeometry, role: FieldArtRole = "actor", width = .9) {
  ...
  const scale: number = role === "weapon" && geometry.heldScale
    ? measureFieldArt(AME_ART.geometry).scale * geometry.heldScale
    : Math.min((role === "door" ? 1.12 : width) / w, (role === "actor" || role === "door" ? 1.35 : .9) / h);
...
export function fieldActorStyle(geometry: ArtGeometry, rowsAbove = Infinity, labelRoom = .36): CSSProperties {
  ...
  return { "--field-label-top": `${Math.max(measureFieldArt(geometry).head - .04, labelRoom - rowsAbove) * 100}%`,
